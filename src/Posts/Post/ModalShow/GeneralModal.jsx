...
const GeneralModal = ({element}) => {
  const navigate = useNavigate();
  const { NewComentOnPost, GetUpdate,DeleteSinglePost, General, Logged, LocalId } = useFirebase();
  ...
  React.useEffect(()=>{
    const fetchHtml = `${element.GeneralTxT}`
  ...
  const DeletePost = () => {
    if (window.confirm("Deseja deletar este Post?")) {
      ...
      DeleteSinglePost(element.id, "General");
    }
  };
  ...
  function NewComent() {
    ...
    NewComentOnPost(CompleteComent, "General", element.id);
  ...
  return (
    ...
    <div className={styles.BackDiv}>
      <div className={styles.TransformeDiv}>
        ...
        {
          element.ImgUrl && <div className={styles.imgSide}> <img src={element.ImgUrl} alt="" /> </div>
        }
            ...
            >
              <div className={styles.Coments}>
                {comentts.map((coment, index) => (
                  <Coments coment={coment} index={index} type={"General"} />
                ))}
              </div>
